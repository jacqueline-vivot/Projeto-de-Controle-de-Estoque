#!/usr/bin/env python3
# inventory_app_moderno.py
# Sistema de Controle de Estoque (Tkinter, SQLite) - Layout moderno, abas, resumo e relatórios
...
# Try import matplotlib for reports; if missing, we'll disable graphs gracefully
HAS_MPL = True
try:
    import matplotlib
...
def parse_date_str(s):
    # Expect YYYY-MM-DD, else return None
    s = (s or "").strip()
    if not s:
        return None
    try:
...
class InventoryApp:
    ...
    def _create_styles(self):
        ...
        style.configure("Header.TLabel", font=("Segoe UI", 13, "bold"), background="#2b6ea3", foreground="white")
        style.configure("Card.TFrame", background="white", relief="flat")
        style.configure("Accent.TButton", foreground="white", background="#2b6ea3", padding=6)
        style.map("Accent.TButton", background=[("active", "#254f80")])
    ...
    def _create_header(self):
        header = ttk.Frame(self.root, padding=0)
        header.pack(fill="x")
        lbl = ttk.Label(header, text="  📦  Sistema de Controle de Estoque", style="Header.TLabel", anchor="w")
        lbl.pack(fill="x")
        # small toolbar
        ...
        if HAS_MPL:
            ...
            # show small hint
            btn = ttk.Button(toolbar, text="Gráficos (opcional)", command=self.missing_matplotlib_info)
    ...
    def _build_dashboard_tab(self):
        frame = ttk.Frame(self.tab_dashboard, padding=12)
        frame.pack(fill="both", expand=True)

        top = ttk.Frame(frame)
        top.pack(fill="x")
        # cards
        self.card_total_products = self._make_card(top, "Total de Produtos", "0")
        self.card_total_stock = self._make_card(top, "Unidades em Estoque", "0")
        self.card_low_stock = self._make_card(top, "Produtos com Estoque Baixo", "0")
        for c in (self.card_total_products, self.card_total_stock, self.card_low_stock):
            c.pack(side="left", padx=8, pady=6, expand=True, fill="x")

        # recent transactions and low stock list
        bottom = ttk.Frame(frame)
        bottom.pack(fill="both", expand=True, pady=(12,0))

        left = ttk.Frame(bottom)
        left.pack(side="left", fill="both", expand=True, padx=(0,6))
        ttk.Label(left, text="Entradas/Saídas recentes", font=("Segoe UI", 11, "bold")).pack(anchor="w")
        self.rv_recent = ttk.Treeview(left, columns=("prod","type","qty","date"), show="headings", height=10)
        ...
        ttk.Label(right, text="Produtos com estoque baixo", font=("Segoe UI", 11, "bold")).pack(anchor="w")
        self.lv_low = ttk.Treeview(right, columns=("name","qty"), show="headings", height=10)
    ...
    def _make_card(self, parent, title, value):
        ...
        lbl = ttk.Label(f, text=value, font=("Segoe UI", 18, "bold"))
    ...
    def _build_inventory_tab(self):
        container = ttk.Frame(self.tab_inventory, padding=12)
        ...
        ttk.Button(top, text="Pesquisar", command=self.refresh_inventory_list).pack(side="left")
        ...
        self.tree_inv.heading("name", text="Nome")
        self.tree_inv.heading("qty", text="Qtd")
        self.tree_inv.heading("price", text="Preço")
        self.tree_inv.heading("expiry", text="Validade")
        self.tree_inv.column("id", width=50, anchor="center")
        self.tree_inv.column("name", width=360)
        self.tree_inv.column("qty", width=80, anchor="center")
        self.tree_inv.column("price", width=120, anchor="center")
        ...
        ttk.Button(footer, text="Entrada", command=lambda: self.open_stock_dialog("in")).pack(side="left", padx=4)
        ttk.Button(footer, text="Saída", command=lambda: self.open_stock_dialog("out")).pack(side="left", padx=4)
    ...
    def _build_transactions_tab(self):
        container = ttk.Frame(self.tab_transactions, padding=12)
        container.pack(fill="both", expand=True)
    ...
    def _build_reports_tab(self):
        container = ttk.Frame(self.tab_reports, padding=12)
        container.pack(fill="both", expand=True)
        # top controls
        top = ttk.Frame(container)
        top.pack(fill="x")
        ...
        if not HAS_MPL:
            msg = ("matplotlib não está instalado. Para habilitar gráficos execute:\n\n"
                   "pip install matplotlib\n\n"
                   "O restante do sistema funciona normalmente.")
            ttk.Label(container, text=msg, foreground="gray").pack(pady=12)
            return
    ...
    def refresh_dashboard(self):
        ...
        # low stock threshold (example: <=5)
        low = [p for p in prods if p["quantity"] <= 5]
        self.card_total_products.value_label.config(text=str(total_products))
        self.card_total_stock.value_label.config(text=str(total_units))
        self.card_low_stock.value_label.config(text=str(len(low)))
    ...
    def get_selected_inventory_id(self):
        ...
        if not sel:
            messagebox.showinfo("Seleção", "Selecione um produto.")
            return None
        return int(sel[0])
    ...
    def update_report_chart(self):
        if not HAS_MPL:
            return
        days = max(1, safe_int(self.report_days.get(), 30))
    ...
    def missing_matplotlib_info(self):
        messagebox.showinfo("Matplotlib ausente",
                            "Para habilitar gráficos instale matplotlib:\n\npip install matplotlib\n\n"
